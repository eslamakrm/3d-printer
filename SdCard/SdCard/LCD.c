...
{
	//set address at CGRAM 
	address |= 0x40;
	LCD_sendCommand(address); //jump to CGRAM location
	// write data 
for(int i=0;i<8;i++)// 5*8 dots for each character
...
void LCD_LOAD_SPECAIAL_CHARACTERS()
{
	...
	{
	//set address at CGRAM 
	 //jump to CGRAM location
	// write data 
	for(int i=0;i<8;i++)// 5*8 dots for each character
           LCD_displayCharacter(S_CHAR[a][i]);
	}
	LCD_sendCommand(0x80);	 // point to location 0 on DDRAM
}
void LCD_Display_smily()
{
	LCD_displayCharacter(SMILY);
}
void LCD_Display_degree()
{
	LCD_displayCharacter(DEGREE);
}
void LCD_Display_midfingure()
{
	LCD_displayCharacter(M_FINGURE);
}
void LCD_Display_SadFace()
{
	LCD_displayCharacter(Sad_face);
}
#endif
#ifdef battery
void LCD_LOAD_BATTERY_SYMBOLS()
{
	uint8 address = 0x40;// set address at CGRAM 
	 //jump to CGRAM location
	LCD_sendCommand(address);
	for(int a=0;a<7;a++)
	{
	...
	// write data 
	for(int i=0;i<8;i++)// 5*8 dots for each character
           LCD_displayCharacter(battary[a][i]);
	}
...
{
	int x = 0;
	while(1)  //put condition to stop the loop
		{
		x = x%7;
	    LCD_displayCharacter(x++);
	    LCD_SHIFT_CURSOR(LEFT,1);
	    delay(250);
		}
}
